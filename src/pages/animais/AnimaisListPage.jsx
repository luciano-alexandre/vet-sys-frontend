...
export default function AnimaisListPage() {
  ...
  async function load() {
    try {
      setError("");
      const data = await animaisApi.list({ q });
      setRows(data);
  ...
  async function remove(id) {
    try {
      ...
      setError(
        "Não foi possível excluir o animal. Verifique se ele está vinculado a algum atendimento."
      );
    }
  }
  ...
  return (
    <div className="page">
      <div className="page-head">
        <h2>Animais</h2>
        <Link className="btn btn-primary" to="/animais/novo">
          Novo animal
        </Link>
      </div>
      ...
      <div className="toolbar">
        <input
          placeholder="Buscar por nome/especie/raca"
          value={q}
      ...
      <Table
        ...
        actions={(r) => (
          <div className="row-actions">
            <Link className="btn" to={`/animais/${r.id}/editar`}>
              Editar
            </Link>
